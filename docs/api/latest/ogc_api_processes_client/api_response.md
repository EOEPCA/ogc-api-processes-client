# Module ogc_api_processes_client.api_response
...
* ogc_api_processes_client.api_response.ApiResponse[LandingPage]
* ogc_api_processes_client.api_response.ApiResponse[ConfClasses]
* ogc_api_processes_client.api_response.ApiResponse[StatusInfo]
* ogc_api_processes_client.api_response.ApiResponse[Execute200Response]
* ogc_api_processes_client.api_response.ApiResponse[JobList]
* ogc_api_processes_client.api_response.ApiResponse[Process]
* ogc_api_processes_client.api_response.ApiResponse[ProcessList]
* ogc_api_processes_client.api_response.ApiResponse[Dict[str, InlineOrRefData]]
...
| union_format | None | The format to use when combining schemas from unions together. Can be one of:<br>- `'any_of'`: Use the [`anyOf`](https://json-schema.org/understanding-json-schema/reference/combining#anyOf)<br>keyword to combine schemas (the default).<br>- `'primitive_type_array'`: Use the [`type`](https://json-schema.org/understanding-json-schema/reference/type)<br>keyword as an array of strings, containing each type of the combination. If any of the schemas is not a primitive<br>type (`string`, `boolean`, `null`, `integer` or `number`) or contains constraints/metadata, falls back to<br>`any_of`. | None |
...
| extra | None | Whether to ignore, allow, or forbid extra data during model validation.<br>See the [`extra` configuration value][pydantic.ConfigDict.extra] for details. | None |
...
| exclude_computed_fields | None | Whether to exclude computed fields.<br>While this can be useful for round-tripping, it is usually recommended to use the dedicated<br>`round_trip` parameter instead. | None |
...
| ensure_ascii | None | If `True`, the output is guaranteed to have all incoming non-ASCII characters escaped.<br>If `False` (the default), these characters will be output as-is. | None |
